...
- Identify which _type_ model to use, and what _engine_ to run it with
...
- An additional `set_engine()` function gives the details on _how_ it should be fit. 
...
* Have multiple interfaces (e.g. formula, x/y, recipe, etc). 
...
* The `predict()` method should give standardized, predictable results. Standardized prediction types (`type = "prob"` for class probabilities `r emojifont::emoji('smile')`). 
...
Via recipe steps and other functions (e.g., backwards feature selection).
